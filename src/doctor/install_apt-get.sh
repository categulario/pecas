...
instalar() {
	if hash $1 2>/dev/null; then
		...
		if [ $2 = "Pandoc" ]; then
			...
			sudo apt-get install tesseract-ocr && sudo apt-get install tesseract-ocr-spa
		elif [ $2 = "Pecas" ]; then
...
# Git
instalar git "Git" "sudo apt-get install git-all"

# Wget
instalar wget "Wget" "sudo apt-get install wget"

# Ruby
instalar ruby "Ruby" "sudo apt-get install ruby-full"

# Pandoc
instalar pandoc "Pandoc"

# Ghostscript
instalar gs "Ghostscript" "sudo apt-get install ghostscript"

# Tesseract
instalar tesseract "Tesseract"

# Scan Tailor
instalar scantailor "Scan Tailor" "sudo apt-get install scantailor"
...
if hash geany 2>/dev/null; then
	echo "Geany ha sido detectado, continuando con las instalaciones..."
else
	if ask "¿Quieres instalar Geany, un editor de código?" Y; then
		sudo apt-get install geany
	else
		echo "Geany no ha sido instalado."
	fi
fi
