...
def beautifier archivo
	...
	def espaciar elemento
		...
		# <head>, <body>, </head> y </body> son nivel 1
			...
			$beautifier_nivel = 1
		# </div>, </blockquote>, </ol>, </ul>, </table>, </tr>, </thead>, </tbody>, </tfoot> y </colgroup> restan un nivel
		elsif elemento =~ /<\/(\s+)?div/ || elemento =~ /<\/(\s+)?blockquote/ || elemento =~ /<\/(\s+)?ol/ || elemento =~ /<\/(\s+)?ul/ || elemento =~ /<\/(\s+)?table/ || elemento =~ /<\/(\s+)?tr/ || elemento =~ /<\/(\s+)?thead/ || elemento =~ /<\/(\s+)?tbody/ || elemento =~ /<\/(\s+)?tfoot/ || elemento =~ /<\/(\s+)?colgroup/
		...
		if elemento =~ /<(\s+)?head/ || elemento =~ /<(\s+)?body/
			$beautifier_nivel = 2
		# Lo que sigue a <div>, <blockquote>, <ol>, <ul>, <table>, <tr>, <thead>, <tbody>, <tfoot> y <colgroup> sube un nivel
		elsif elemento =~ /<(\s+)?div/ || elemento =~ /<(\s+)?blockquote/ || elemento =~ /<(\s+)?ol/ || elemento =~ /<(\s+)?ul/ || elemento =~ /<(\s+)?table/ || elemento =~ /<(\s+)?tr/ || elemento =~ /<(\s+)?thead/ || elemento =~ /<(\s+)?tbody/ || elemento =~ /<(\s+)?tfoot/ || elemento =~ /<(\s+)?colgroup/
			$beautifier_nivel = $beautifier_nivel + 1
		end
	end
