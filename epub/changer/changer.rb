...
if version_actual.to_i == 3
	...
	archivo.puts JSON.pretty_generate(hash_to_html(epub_objeto["opf"]))
	archivo.close
...
puts "\nCreando EPUB versión #{epub_version}…".green

# Crea el EPUB
system ("#{zip} #{arregloRutaTerminal($l_g_epub_analisis)} -X mimetype")
